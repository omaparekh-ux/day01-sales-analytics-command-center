...
def generate(n: int = 100) -> pd.DataFrame:
    """Generate n reproducible transactions with documented commercial relationships."""
    ...
    units = rng.poisson(np.where(segment == "Enterprise", 4, np.where(segment == "SMB", 3, 2))) + 1
    price = np.array([base_price[p] for p in product]) * rng.normal(1, 0.06, n)
    discount = np.clip(rng.normal(np.where(channel == "Distributor", 0.16, np.where(segment == "Enterprise", 0.11, 0.07)), 0.035, n), 0, 0.30)
    ...
    return pd.DataFrame({
        "order_id": [f"ORD-{i:06d}" for i in range(1, n + 1)], "order_date": order_date,
        "region": region, "channel": channel, "segment": segment, "category": category,
        "product": product, "units": units, "unit_price": np.round(price, 2),
        "discount_pct": np.round(discount, 4), "unit_cost": np.round(cost, 2),
    })
