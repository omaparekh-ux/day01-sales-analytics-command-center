...
from .analytics import aggregate, anomalies, enrich, kpis, load_sales, pareto, product_economics, recommendations, validate_sales, monthly_trend, yoy_comparison
...
def build(source: Path, destination: Path) -> dict:
    ...
    artifact = {
        ...
        "health": {"status": "healthy", "validated_rows": len(df), "validated_at_build": True},
        ...
        "dimensions": {dim: _safe_records(aggregate(df, dim)) for dim in ["category", "region", "channel", "segment"]},
        "recommendations": recommendations(df),
    }
    destination.parent.mkdir(parents=True, exist_ok=True)
    destination.write_text(json.dumps(artifact, indent=2, sort_keys=True, allow_nan=False), encoding="utf-8")
    return artifact
